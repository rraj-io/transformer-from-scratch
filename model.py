...
class MultiHeadAttentionBlock(nn.Module):
    ...
    @staticmethod
    def attention(query, key, vlaue, mask, dropout: nn.Dropout):
        d_k = query.shape[-1]
...
class DecoderBlock(nn.module):
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask):
        ...
        x = self.residual_connections[1](x, lambda: self.self_attention_block(x, encoder_output, encoder_output, src_mask))
...
def build_transformer(src_vocab_size: int,
    ...
    # create teh embedding layers
    src_embed = InputEmbeddings(d_model, src_vocab_size)
    tgt_embed = InputEmbeddings(d_model, tgt_vocab_size)

    # CCraete teh positional encodings layers
    src_pos = PositionalEncoding(d_model, src_seq_len, dropout)
    tgt_pos = PositionalEncoding(d_model, tgt_seq_len, dropout)

    # CCreate the encoder blocks 
    encoder_blocks = []
    for _ in range(N):
    ...
    encoer = Encoder(nn.ModuleList(encoder_blocks))
